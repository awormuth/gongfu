...
func RequireAuth() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if hasAccess == false {
			...
			c.Redirect(http.StatusForbidden, "https://hex.care")
...
func main() {
	...
	// router1 := g.Group("/webapp")
	// router2 := g.Group("/html")

	// router1.Static("/*filepath", "webapp")
	// router2.Static("/*filepath", "webapp/html")

	g.Static("/webapp", "webapp")
	g.Static("/html", "webapp/html")

	// router.Static("/webapp/*filepath", "webapp")
	// router.Static("/html/*filepath", "webapp/html")

	log.Printf("... started ServeFile ...")
	log.Printf("... serving %v TLS on port %v ", cfg.APPNAME, cfg.HTTP_PORT)
	g.RunTLS(cfg.HTTP_PORT, "certs/hexcare.crt", "certs/hexcare.key")
