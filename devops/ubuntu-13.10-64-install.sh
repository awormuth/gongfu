...
fn_install_go () {
    #golang go 1.3
    sudo mkdir /opt/go
    sudo mkdir -p /home/ubuntu/go/bin
    sudo mkdir -p /home/ubuntu/go/pkg
    sudo mkdir -p /home/ubuntu/go/src
    ...
    echo export GOPATH=/home/ubuntu/DoctorPortal >> ~/.profile
...
fn_install_dev_tools () {
    # This directory is set in ubuntufile (config.vm.synced_folder)
...
fn_check_for_previous_install () {
    ...
    then
        ...
        echo "To run system updates manually login via 'ubuntu ssh' and run 'apt-get update && apt-get upgrade'"
        echo ""
        # fn_print_db_usage
        exit
    fi
